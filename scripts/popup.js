...
// Send a message to the active tab to 'acceptrequest' it
function sendAcceptrequest() {
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) { // Finds tabs that are active in the current window
    chrome.tabs.sendMessage(tabs[0].id, { action: 'acceptrequest' }); // Sends a message (object) to the first tab (tabs[0])
  });
}
...
document.querySelector('#acceptrequest').addEventListener('click', event => sendAcceptrequest());
